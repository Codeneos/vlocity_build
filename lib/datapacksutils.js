...
var DataPacksUtils = module.exports = function(vlocity) {
    ...
    this.perfTimers = {};
    this.namespacePlaceholders = {
        vlocity: { 
            search: ['vlocity_namespace', 'vlocity', 'vlc'],
            prefix: '%vlocity_namespace%__'
        },
        custom: { 
            search: ['custom_namespace', 'custom', 'cst'],
            prefix: ''
        }
    };
    this.namespacePlaceholders.default = this.namespacePlaceholders.vlocity;
};
...
DataPacksUtils.prototype.updateExpandedDefinitionNamespace = function(currentData) {
    var self = this;
     ...
     if (Array.isArray(currentData)) {
        return currentData.map(function(value) {
            return self.updateExpandedDefinitionNamespace(value);
        });
    }
    ...
    if (typeof currentData === 'object') {
        ...
        for (var key in currentData) {
            if(currentData.hasOwnProperty(key)) {
                replacedObject[self.updateExpandedDefinitionNamespace(key)] = 
                    self.updateExpandedDefinitionNamespace(currentData[key]);
            }
        }
    ...
    if (typeof currentData === 'string') {
        return currentData
            .split('.')
            .map(function(field) {
                // skip default salesforce fields
                if (!field.endsWith('__c') && !field.endsWith('__r'))
                    return field;

                // for custom fields replace based on the namesspace place holder config
                var placeholder = self.namespacePlaceholders.default;
                var namespacePrefix = field.match(/^(%|)([A-Z0-9_]+)(%|)__(.+?)__(c|r)$/i);

                if (namespacePrefix) {                    
                    for(var key in self.namespacePlaceholders) {
                        if (self.namespacePlaceholders[key].search.includes(namespacePrefix[2])) {
                            placeholder = self.namespacePlaceholders[key];
                            break;
                        }                                             
                    };
                }

                return placeholder.prefix + field;
            })
            .join('.');
    }
...
DataPacksUtils.prototype.overrideExpandedDefinition = function(expandedDefinition) {
    this.overrideExpandedDefinitions = this.updateExpandedDefinitionNamespace(expandedDefinition);    
