...
DataPacksExpand.prototype.generateFolderOrFilename = function(filename, extension) {
    ...
    return path.normalize(sanitizedFilename);
}
...
DataPacksExpand.prototype.sanitizeDataPackKey = function(key) {
    ...
    return beforeSlash + '/' + this.generateFolderOrFilename(key).replace(path.sep, '/');
}
...
DataPacksExpand.prototype.getNameWithFields = function(nameFields, dataPackData) {
    ...
    var processedNames = nameFields.map(function(key) {
        ...
        if (key.indexOf('_') == 0) {
            ...
            return dataPackData[key].replace(/[\\\/]+/g, '');
...
DataPacksExpand.prototype.generateSourceKey = function(currentData, jobInfo) {
    ...
    return self.vlocity.datapacksutils.guid(generatedKey);
}
    ...
    if (currentData) {
        ...
        if (currentData.VlocityDataPackData) {
            ...
            if (Array.isArray(currentData)) {
                ...
                //self.vlocity.datapacksutils.runValidationRules(currentData, dataPackType, dataPackKey, jobInfo) ;
...
DataPacksExpand.prototype.writeFile = function(dataPackType, parentName, filename, fileType, fileData, isPagination, encoding) {
    ...
    if (fileType == "json") {
        if (typeof fileData === "object") {
            ...
            try {
                ...
                VlocityUtils.log('\x1b[31m', "Error", '\x1b[0m ', filename + "." + fileType, e);
                return fileData;
            }
        }
    ...
    if (fullFilePath.indexOf('_DataPack') > -1 || self.vlocity.verbose) {
         VlocityUtils.log('\x1b[32m', 'Creating file >>', '\x1b[0m', path.normalize(fullFilePath));
    }
