...
var MAX_IMPORT_PRE_STEP_LENGTH = 10000;
...
DataPacksBuilder.prototype.buildImport = function(importPath, manifest, jobInfo, onComplete) {
    ...
    if (self.vlocity.verbose) {
        VlocityUtils.log('\x1b[31m', 'buildImport >>' ,'\x1b[0m', importPath, jobInfo.manifest, jobInfo);
    }
    ...
    var currentImportPreStepLength = 0;
    ...
    if (jobInfo.expansionPath) {
        importPath = path.resolve(path.join(importPath, jobInfo.expansionPath));
    }
    ...
    do {
        ...
        if (nextImport) {
            currentImportPreStepLength += stringify(jobInfo.allDeployDataSummary[nextImport.VlocityDataPackKey]).length;

           if (!jobInfo.singleFile && Object.keys(currentDataPackKeysInImport).length > 1 && (stringify(nextImport).length > maxImportSize || currentImportPreStepLength > MAX_IMPORT_PRE_STEP_LENGTH)) {
            ...
            if (recordsCounts > 0) {
                ...
                dataPackParent[keyField].forEach(function(childRecord) {
                    ...
                    if (currentCount != 0 && (currentCount + thisRecordCount) > paginationLimit) {
                        ...
                        if (!jobInfo.noStatus) {
                            VlocityUtils.log('\x1b[32m', 'Adding to ' + (jobInfo.singleFile ? 'File' : 'Deploy') + ' >>', '\x1b[0m', paginatedDataPack.VlocityDataPackKey + ' - ' + paginatedDataPack.VlocityDataPackLabel, '\x1b[31m', jobInfo.headersOnly ? 'Headers Only' : '', jobInfo.forceDeploy ? 'Force Deploy' : '', '\x1b[0m');
                        }
...
DataPacksBuilder.prototype.getDataPackLabel = function(dataPackTypeDir, dataPackName) {
    try {        
        var par = path.join(dataPackTypeDir, dataPackName);
        var allFiles = this.vlocity.datapacksutils.getFiles(path.join(dataPackTypeDir, dataPackName));
        for (var i = 0; i < allFiles.length; i++) {
            var basename = this.vlocity.datapacksutils.getBaseName(allFiles[i]);
...
DataPacksBuilder.prototype.initializeImportStatus = function(importPath, manifest, jobInfo) {
    ...
    if (!self.vlocity.datapacksutils.fileExists(importPath)) {
        VlocityUtils.log('\x1b[32m', 'No Data At Path >>' ,'\x1b[0m', importPath);
        return;
    }

    var importPaths = self.vlocity.datapacksutils.getDirectories(importPath);
    if (self.vlocity.verbose) {
        VlocityUtils.log('\x1b[31m', 'Found import paths >>' ,'\x1b[0m', importPaths);
    }

    importPaths.forEach(function(dataPackType) {
        var dataPackTypeDir = path.join(importPath, dataPackType);
        var allDataPacksOfType = self.vlocity.datapacksutils.getDirectories(dataPackTypeDir, true);

        if (self.vlocity.verbose) {
            VlocityUtils.log('\x1b[31m', 'Found datapacks >>' ,'\x1b[0m', allDataPacksOfType);
        }
        ...
        if (allDataPacksOfType) {
            allDataPacksOfType.forEach(function(dataPackName) {                
                ...
                if(dataPackLabel == null)
                    return;
                    
                // Manifest check
                if (manifest != null && manifest[dataPackType] && !matchingFunction(dataPackType, dataPackName))
                    return;
                    
                try {
                    self.loadFilesAtPath(path.join(dataPackTypeDir, dataPackName), jobInfo, dataPackKey);
                    var sobjectData = JSON.parse(self.getFileData(dataPackTypeDir, dataPackName, dataPackLabel + '_DataPack.json'));                        
                    ...
                    VlocityUtils.error('\x1b[31m', 'Error whilst processing >>' ,'\x1b[0m', dataPackKey, e.stack);
                }
            });
        }
    });
    ...
    if (hasMissingEntries) {
        VlocityUtils.error("Unmatched but required files:\n" + stringify(self.pendingFromManifest, null, 2));
    }
};
    ...
    for (var i = 0; i < dataPackKeys.length; i++) {
        ...
        if (jobInfo.currentStatus[dataPackKey] == 'Ready' || (jobInfo.currentStatus[dataPackKey] == 'ReadySeparate' && Object.keys(currentDataPackKeysInImport).length == 0) || (jobInfo.currentStatus[dataPackKey] == 'Header' && !jobInfo.headersOnly && Object.keys(currentDataPackKeysInImport).length == 0)) {
            try {
                ...
                var dataPackLabel = self.getDataPackLabel(path.join(importPath, dataPackType), self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName));

                var fullPathToFiles = path.join(importPath, dataPackType, self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName));
                var parentData;
                ...
                if (!jobInfo.headersOnly && !jobInfo.forceDeploy) {
                    var parentFileNameFull = self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName + "_ParentKeys", "json");
                    parentData = self.getFileData(importPath, dataPackType, parentFileNameFull);
                ...
                if (!jobInfo.noStatus) {
                    VlocityUtils.log('\x1b[32m', 'Adding to ' + (jobInfo.singleFile ? 'File' : 'Deploy') + ' >>', '\x1b[0m', nextImport.VlocityDataPackKey + ' - ' + dataPackLabel, '\x1b[31m', jobInfo.headersOnly ? 'Headers Only' : '', jobInfo.forceDeploy ? 'Force Deploy' : '', '\x1b[0m');
                }
                ...
                VlocityUtils.log('\x1b[31m', 'Error Formatting Deploy >>','\x1b[0m', dataPackKey, e.stack);
                throw e;
            }
        }
    }
        ...
        Object.keys(dataPackData).forEach(function(field) {            
            ...
            if (field != 'Name') {
                    ...
                    && typeof potentialFileNames[0] === 'string' ) 
                {
                    ...
                    potentialFileNames.forEach(function(file) {
                        var fileData = self.getFileData(fullPathToFiles, file);
                        if (fileData) {
                             allDataPackFileData = allDataPackFileData.concat(self.buildFromFiles(JSON.parse(fileData), fullPathToFiles, dataPackType, field));
                        } else {
                            VlocityUtils.log('\x1b[31m', 'File Does Not Exist >>' ,'\x1b[0m', path.join(fullPathToFiles, file));
                        }
                    });
                            ...
                            if (self.compileOnBuild && fileType.CompiledField) { 
                                ...
                                self.compileQueue.push({
                                    ...
                                    callback: function(error, compiledResult) {
                                        if (error) {
                                            return VlocityUtils.log('\x1b[31m', 'Failed to compile SCSS for >>' ,'\x1b[0m', filename, '\n', error.message);
                                        }
    ...
    var compileNext = (job) => {
        if (!job) {
            if (this.vlocity.verbose && (compileCount > 0 || errors.length > 0)) {
                VlocityUtils.log('\x1b[31m', 'Compilation done >>' ,'\x1b[0m', 'compiled', compileCount, 'files with', errors.length, 'errors.');
            }
            return onComplete({ 
        ...
        if (this.vlocity.verbose) {
            VlocityUtils.log('\x1b[31m', 'Start compilation of >>' ,'\x1b[0m', job.filename);
        }

        this.compile(job.language, job.source, job.options || {}, (error, compiledResult) => {
            if(error) {
                VlocityUtils.log('\x1b[31m', error.message, '>>' ,'\x1b[0m', '\n');
