...
DataPacksBuilder.prototype.buildImport = function(importPath, jobInfo, onComplete) {
    var self = this;
    ...
    do {
        ...
        if (nextImport) {
            currentImportPreStepLength += stringify(jobInfo.allDataSummary[nextImport.VlocityDataPackKey]).length;
            ...
            if (recordsCounts > 0) {
                if (currentSObject[keyField].length > 0 && currentSObject[keyField].length < paginationLimit) {
                    ...
                    for (var i = 0; i < arrayOfPaginated.length; i++) {
                        ...
                        //VlocityUtils.success('Adding to ' + (jobInfo.singleFile ? 'File' : 'Deploy') + ' Paginated', addingPaginatedDataPack.VlocityDataPackKey, jobInfo.headersOnly ? '- Headers Only' : '', jobInfo.forceDeploy ? '- Force Deploy' : '');
        ...
        allDataPacksOfType.forEach(function(dataPackName) {
            ...
            if (self.isInManifest(jobInfo, dataPackType, generatedDataPackKey, dataPackName)) {
                ...
                self.vlocity.datapacksutils.getApexImportDataKeys(sobjectData.VlocityRecordSObjectType).forEach(function(field) {
                    apexImportData[field] = sobjectData[field] || sobjectData['%vlocity_namespace%__' + field];
                });
    ...
    for (var i = 0; i < dataPackKeys.length; i++) {
        ...
        if (jobInfo.currentStatus[dataPackKey] == 'Ready' 
            ...
            try {
                ...
                var dataPackLabel = self.getDataPackLabel(importPath + '/' + dataPackType, self.vlocity.datapacksexpand.generateFolderOrFilename(dataPackName));
