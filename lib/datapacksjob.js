...
DataPacksJob.prototype.runStepApex = function(projectPath, stepSettings, apexData, shouldDebug, onComplete) {
    ...
    if (stepSettings) {
        ...
        apexData.forEach(function(dataPack) {
            ...
            if (apexClass) {
                var currentBatches = runApexByType.length-1;
                var MAX_ANON_APEX_CHARS = 8000;
                if (currentBatches == -1 
                    || runApexByType[currentBatches].apexClass != apexClass
                    || stringify(runApexByType[currentBatches].apexData).length > MAX_ANON_APEX_CHARS) {
                    runApexByType.push({ apexClass: apexClass, apexData: [ dataPack ]});
                } else {
                    runApexByType[currentBatches].apexData.push(dataPack);
                }
            }
        });
        
        async.eachSeries(runApexByType, function(apexBatch, callback) {
            self.vlocity.datapacksutils.runApex(projectPath, apexBatch.apexClass, apexBatch.apexData).then(function() {
                callback();
            });
        }, function(err, result) {
            onComplete();
