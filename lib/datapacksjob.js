...
DataPacksJob.prototype.runJob = function(jobData, jobName, action, onSuccess, onError) {
    ...
    self.vlocity.checkLogin(function() {
        ...
        try {
            ...
            VlocityUtils.error('Initialization Error', e.message);
            jobInfo.hasError = true;
            jobInfo.errors.push(e);
...
DataPacksJob.prototype.doRunJob = function(jobInfo, action, onComplete) {
    ...
    if (action == 'Export') {
        ...
        self.reExpandAllFiles(jobInfo, onComplete);
        ...
        jobInfo.hasError = true;
        jobInfo.message = 'Command not found: ' + action;
        VlocityUtils.error(jobInfo.message);
...
DataPacksJob.prototype.formatResponse = function(jobInfo, error) {
    ...
    Object.keys(jobInfo.fullManifest).forEach(function(dataPackType) {
        Object.keys(jobInfo.fullManifest[dataPackType]).forEach(function(dataPackKey) {
            allManifestByKey[dataPackKey] = jobInfo.fullManifest[dataPackType][dataPackKey];
    ...
    if (jobInfo.jobAction == 'GetAllAvailableExports') {
        Object.keys(allManifestByKey).map((k) => allManifestByKey[k]).forEach(function(item) {
            VlocityUtils.log(item.VlocityDataPackKey);
        });

        response.records = Object.keys(allManifestByKey).map((k) => allManifestByKey[k]);
        ...
        response.records = Object.keys(dataPacksMap).map((k) => dataPacksMap[k]);
...
DataPacksJob.prototype.formatManifest = function(jobInfo) {
    ...
    if (Array.isArray(jobInfo.manifest)) {
        ...
        jobInfo.queries = jobInfo.queries.concat(Object.keys(self.queryDefinitions).map((k) => self.queryDefinitions[k]));
    } else {
            ...
            var thisQuery = self.vlocity.jsForceConnection.query(query)
                .on("record", function(record) {
                    ...
                    if (!jobInfo.fullManifest[queryData.VlocityDataPackType][record.Id]) {
                        if (jobInfo.specificManifestKeys && (jobInfo.specificManifestKeys.indexOf(record.VlocityDataPackKeyForManifest) == -1 && jobInfo.specificManifestKeys.indexOf(record.Id) == -1)) {
                            return;
                        }
                    ...
                    self.vlocity.datapacks.getDataPackData(result.VlocityDataPackId, function(dataPackData) {
                        ...
                        (dataPackData.dataPacks || []).forEach(function(dataPack) {
                            ...
                            if (dataPack.VlocityDataPackStatus == 'Success') {
                                ...
                                } else if (!jobInfo.fullManifest[dataPack.VlocityDataPackType][dataPack.VlocityDataPackKey] && !jobInfo.specificManifestKeys) {
                            ...
                            } else if (dataPack.VlocityDataPackRelationshipType != 'Children' && (dataPack.VlocityDataPackStatus == 'Error' || dataPack.VlocityDataPackStatus == 'Ignored')) {
                        ...
                        originalKeys.forEach(function(pack){
                            var packKey = pack.VlocityDataPackKey ? pack.VlocityDataPackKey : pack.VlocityDataPackKeyForManifest;
                            ...
                            } else if (packKey && !processedKeys[packKey]) {
                                VlocityUtils.verbose('Ignored', packKey);
                                jobInfo.currentStatus[packKey] = 'Ignored';
                            } else if (pack.Id && !processedKeys[pack.Id]) {
                                if (jobInfo.alreadyErroredIdsByType[pack.VlocityDataPackType] == null) {
                                    jobInfo.alreadyErroredIdsByType[pack.VlocityDataPackType] = [];
                                }
...
DataPacksJob.prototype.buildImportSeries = function(jobInfo, deploySeries, onComplete) {
    ...
    self.vlocity.datapacksbuilder.buildImport(jobInfo.projectPath, jobInfo, function(deployEntry) {
            ...
            self.vlocity.datapacks.getDataPackData(dataPackData.dataPackId, function(dataPackData) {
                ...
                dataPackData.dataPacks.forEach(function(dataPack) {
                    ...
                    if (dataPack.ActivationStatus == 'Ready' && dataPack.VlocityDataPackStatus == 'Success') {
                        ...
                        var message = 'Activation Error >> ' +  dataPack.VlocityDataPackKey + ' --- ' + dataPack.ActivationMessage ? dataPack.ActivationMessage : '';
                        ...
                        if (jobInfo.errors.indexOf(message) == -1) {
                            ...
                            VlocityUtils.error('Activation Error', dataPack.VlocityDataPackKey, dataPack.ActivationMessage ? dataPack.ActivationMessage : '');
                        }
                    }
                });
...
DataPacksJob.prototype.reExpandAllFiles = function(jobInfo, onComplete) {
