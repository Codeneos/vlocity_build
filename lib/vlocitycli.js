...
VLOCITY_COMMANDLINE_COMMANDS = {
    ...
    refreshProject: {
        name: 'RefreshProject',
        description: 'Refresh the Project to the latest format for this tool'
    },
    runApex: {
        name: 'Apex',
...
VlocityCLI.prototype.runCLI = function(commands, success, error) {
    ...
    if (commands.length == 0) {
        ...
        if (self.isJsonCLI) {
            return success(self.formatResponseCLI({ message: 'Commands', records: Object.keys(VLOCITY_COMMANDLINE_COMMANDS).map((k) => VLOCITY_COMMANDLINE_COMMANDS[k]), action: 'help', status: 'success' }));
        } else {
            return success('');
        }
    }
    ...
    if (commands.indexOf('packUpdateSettings') != -1) {
        ...
        dataPacksJobsData.UpdateSettings = {
